    ...
    return (
      <div style={{ background:"#0A0A0A", color:"#F0EDE8", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Syne',sans-serif" }}>
        <div style={{ textAlign:"center" }}>
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:60, fontWeight:600, color:"#C9A84C", lineHeight:1 }}>404</div>
          <p style={{ color:"#555", marginTop:16, marginBottom:24 }}>Article introuvable</p>
          <button onClick={() => router.push("/blog")} style={{ background:"linear-gradient(135deg,#C9A84C,#A67C2E)", border:"none", color:"#0A0A0A", padding:"12px 28px", fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700, letterSpacing:"2px", textTransform:"uppercase", cursor:"pointer" }}>
            ← Retour au blog
          </button>
        </div>
      </div>
  ...
  function renderContent(text) {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("## ")) {
        ...
        elements.push(
          <h3 key={i} style={{ fontFamily:"'Syne',sans-serif", fontSize:14, fontWeight:700, color:"#C9A84C", letterSpacing:"2px", textTransform:"uppercase", margin:"1.5rem 0 0.75rem" }}>
            {line.replace("### ", "")}
          </h3>
        );
      } else if (line.startsWith("- **")) {
        ...
        elements.push(
          <p key={i} style={{ color:"#E8C87A", fontSize:15, fontWeight:700, lineHeight:1.8, margin:"1rem 0 0.5rem" }}>
            {line.replace(/\*\*/g, "")}
          </p>
        );
      } else if (line.trim() !== "") {
    ...
    <div style={{ background:"#0A0A0A", color:"#F0EDE8", minHeight:"100vh", fontFamily:"'Syne',sans-serif" }}>
      ...
      <div style={{ maxWidth:780, margin:"0 auto", padding:"3rem 1.5rem 2rem" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:"1.5rem" }}>
          <span style={{ fontSize:10, letterSpacing:"2px", textTransform:"uppercase", color:"#C9A84C", background:"rgba(201,168,76,0.08)", border:"0.5px solid rgba(201,168,76,0.2)", padding:"4px 12px", borderRadius:2 }}>
            {article.categorie}
          </span>
        ...
        <div style={{ height:"0.5px", background:"linear-gradient(90deg,#C9A84C,transparent)", marginBottom:"2.5rem" }} />
      </div>
      ...
      <div style={{ maxWidth:780, margin:"3rem auto", padding:"0 1.5rem" }}>
        <div style={{ background:"#0D0D0D", border:"0.5px solid rgba(201,168,76,0.3)", borderRadius:4, padding:"2rem", textAlign:"center" }}>
          <div style={{ fontSize:10, letterSpacing:"3px", textTransform:"uppercase", color:"#C9A84C", marginBottom:"0.75rem" }}>— Prêt à passer à l'action ?</div>
          ...
          <button onClick={() => router.push("/#offres")} style={{ background:"linear-gradient(135deg,#C9A84C,#A67C2E)", border:"none", color:"#0A0A0A", padding:"13px 28px", fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700, letterSpacing:"3px", textTransform:"uppercase", cursor:"pointer", borderRadius:2 }}>
      ...
      {autres.length > 0 && (
        <div style={{ maxWidth:1100, margin:"0 auto", padding:"2rem 1.5rem 5rem" }}>
          <div style={{ fontSize:11, letterSpacing:"3px", textTransform:"uppercase", color:"#C9A84C", marginBottom:"1.5rem" }}>— Articles similaires</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
            {autres.map(a => (
              <article key={a.slug} className="article-card" onClick={() => router.push(`/blog/${a.slug}`)}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                  <span style={{ fontSize:10, letterSpacing:"2px", textTransform:"uppercase", color:"#C9A84C" }}>{a.categorie}</span>
                ...
                <span style={{ fontSize:12, color:"#C9A84C" }}>Lire →</span>
              </article>
            ))}
          </div>
        </div>
