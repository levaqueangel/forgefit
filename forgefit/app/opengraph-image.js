...
export default function OgImage() {
  return new ImageResponse(
    <div style={{ width:"100%", height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#0A0A0A", fontFamily:"sans-serif", position:"relative" }}>
      {/* Background gradient */}
      <div style={{ position:"absolute", width:600, height:600, borderRadius:"50%", background:"radial-gradient(circle, rgba(201,168,76,0.12) 0%, transparent 70%)", top:"50%", left:"50%", transform:"translate(-50%,-50%)", display:"flex" }}/>
      {/* Logo */}
      ...
      <div style={{ display:"flex", gap:32 }}>
        {["Programme Sur Mesure", "Nutrition Calculée", "Résultats Prouvés"].map((label) => (
          <div key={label} style={{ background:"rgba(201,168,76,0.08)", border:"0.5px solid rgba(201,168,76,0.3)", padding:"12px 24px", display:"flex" }}>
            <span style={{ fontSize:13, color:"#C9A84C", letterSpacing:2, textTransform:"uppercase" }}>{label}</span>
          </div>
        ))}
      </div>
      {/* URL */}
