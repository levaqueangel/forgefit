...
export default function NotFound() {
  ...
  return (
    <div style={{ background:"#0A0A0A", color:"#F0EDE8", minHeight:"100vh",
      ...
      `}</style>
      <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 50% 40%,rgba(201,168,76,0.04) 0%,transparent 60%)", pointerEvents:"none" }}/>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:480, width:"100%" }}>
        <div className="a1" style={{ fontSize:10, letterSpacing:"4px", textTransform:"uppercase", color:"#C9A84C", marginBottom:"1rem" }}>{t.tag}</div>
        <h1 className="a2" style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(32px,5vw,52px)", fontWeight:600, lineHeight:1.1, marginBottom:"1rem" }}>
          {t.title}<br/><em style={{ fontStyle:"italic", color:"#C9A84C" }}>{t.subtitle}</em>
        </h1>
        <p className="a3" style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:17, color:"#555", lineHeight:1.9, marginBottom:"1.5rem" }}>
          {t.desc} <strong style={{ color:"#C9A84C" }}>{count}s</strong>.
        </p>
        <div className="a3" style={{ height:2, background:"#1A1A1A", borderRadius:2, marginBottom:"2.5rem", overflow:"hidden" }}>
          <div style={{ height:"100%", background:"linear-gradient(90deg,#C9A84C,#E8C87A)", borderRadius:2, width:`${((10-count)/10)*100}%`, transition:"width 1s linear" }}/>
        </div>
        <div className="a4" style={{ marginBottom:"2rem", display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          <button onClick={() => router.push("/")} style={{ background:"linear-gradient(135deg,#C9A84C,#A67C2E)", border:"none", color:"#0A0A0A", padding:"14px 32px", fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700, letterSpacing:"2px", textTransform:"uppercase", cursor:"pointer" }}>
            {t.btn_home}
          </button>
